...
try:
    ...
    # Add the turing repo root to path
    ...
    # Import LCD communication directly
    from library.lcd.lcd_comm_rev_a import LcdCommRevA
    from library.lcd.lcd_comm_rev_b import LcdCommRevB
    from library.lcd.lcd_comm import LcdComm
    
    # Create a wrapper class that uses the LCD directly
    class TuringSmartScreen:
        def __init__(self, display_type="3.5inch", orientation=0):
            ...
            # Initialize LCD communication directly - try Rev A first since your hardware is Rev A
            try:
                self.lcd = LcdCommRevA()
                print("✅ LCD Rev A initialized!")
            except Exception as e:
                print(f"Rev A failed: {e}, trying Rev B...")
                try:
                    self.lcd = LcdCommRevB()
                    print("✅ LCD Rev B initialized!")
                except Exception as e:
                    print(f"Rev B failed: {e}, trying base LCD...")
                    self.lcd = LcdComm()
                    print("✅ Base LCD initialized!")
        ...
        def display_image(self, image):
            """Send PIL image to the display using DisplayPILImage"""
            try:
                ...
                # Use DisplayPILImage - we know this works!
                self.lcd.DisplayPILImage(image, 0, 0)
                print("✅ Image displayed on LCD!")
                    
            except Exception as e:
                print(f"❌ Error displaying image: {e}")
                # Save for debugging
                filename = f"error_display_{int(time.time())}.png"
                image.save(filename)
                print(f"Saved error image to {filename}")
        ...
        def close(self):
            ...
            try:
                self.lcd.ScreenOff()
                if hasattr(self.lcd, 'closeSerial'):
                    self.lcd.closeSerial()
            except Exception as e:
                print(f"Error closing LCD: {e}")
    
    SCREEN_AVAILABLE = True
    print("✅ Successfully imported LCD communication!")
